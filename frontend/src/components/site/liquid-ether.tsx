...
// Adapted from reactbits.dev's LiquidEther (WebGL fluid simulation background:
// https://reactbits.dev/backgrounds/liquid-ether). Kept close to the original
// GPU pipeline (advection/viscosity/divergence/poisson/pressure shader
// passes) since it's tested physics code; `any` is disabled file-wide because
// the shader-pass classes intentionally pass loosely-shaped uniform bags
// between stages, matching the upstream implementation.
...
export function LiquidEther({
  ...
  const webglRef = React.useRef<LiquidEtherWebGL | null>(null)
...
`
    ...
    class ShaderPass {
      ...
      // Base signatures take a rest param so subclasses (Advection,
      // ExternalForce, etc.) can override with concrete params — TS method
      // override compatibility requires the base accept at least as many.
...
/** Fuel-branded preset: nutrition green / citrus orange / warm gold instead of
 * the upstream purple, at a gentler force so it reads as ambient rather than
 * a toy. Skips mounting only on touch-only devices — WebGL fluid sim has no
 * cursor to react to there, and would just burn battery running the idle
 * auto-demo. (Deliberately does NOT gate on prefers-reduced-motion: this is
 * a decorative hero background, not essential UI motion, and hiding it
 * silently made it look broken/missing rather than intentionally absent.) */
export function FuelLiquidEther({
  className,
  style,
}: {
  className?: string
